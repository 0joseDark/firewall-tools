...
def varrer_portas():
    try:
        ...
        for linha in regras.split('\n'):
            ...
            if "LocalPort" in linha and "UDP" in linha:
                porta_udp = linha.split(":")[1].strip()
                
            # Exibir o formato "programa + porta tcp + porta udp"
            if programa and (porta_tcp or porta_udp):
                lista_portas.insert(tk.END, f"{programa} + TCP: {porta_tcp} + UDP: {porta_udp}")
                log(f"{programa} + TCP: {porta_tcp} + UDP: {porta_udp}")
                programa = ''
                porta_tcp = ''
                porta_udp = ''
