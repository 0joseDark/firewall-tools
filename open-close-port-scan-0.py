...
def varrer_portas():
    try:
        ...
        for linha in regras.split('\n'):
            ...
            if "Protocol" in linha and "TCP" in linha:
                porta_tcp = linha.split(":")[1].strip()
                
            if "Protocol" in linha and "UDP" in linha:
                porta_udp = linha.split(":")[1].strip()
            
            # Exibir o formato "programa + porta udp + porta tcp"
            if programa or porta_tcp or porta_udp:
                entrada = f"{programa} + UDP: {porta_udp} + TCP: {porta_tcp}"
                lista_portas.insert(tk.END, entrada)
                log(entrada)
                # Resetar variáveis para evitar sobreposição
                programa = ''
                porta_tcp = ''
                porta_udp = ''
